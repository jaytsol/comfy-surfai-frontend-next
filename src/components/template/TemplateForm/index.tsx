...
const TemplateForm: React.FC<TemplateFormProps> = ({
  ...
  if (templates.length === 0) {
    return <p className="text-orange-600">사용 가능한 워크플로우 템플릿이 없습니다. 먼저 템플릿을 생성해주세요.</p>;
  }
      ...
      {selectedTemplate && selectedTemplate.parameter_map && Object.keys(selectedTemplate.parameter_map).length > 0 && (
        <form onSubmit={onSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {Object.entries(selectedTemplate.parameter_map).map(([paramName, paramConfig]) => {
              const paramValue = parameterValues[paramName] ?? '';
              const inputType = typeof paramValue === 'boolean' ? 'checkbox' : 
...
export { TemplateForm };
export type { TemplateFormProps } from './types';
