...
const ItemLightbox: React.FC<ItemLightboxProps> = ({ item, onClose }) => {
  // ✨ useEffect와 상태 관리 로직을 모두 제거합니다.

  // item이 없으면 아무것도 렌더링하지 않습니다.
  if (!item) {
    return null;
  }
  ...
  return (
    ...
    >
      <button
        onClick={onClose}
        className="absolute top-4 right-5 text-white text-4xl font-bold ..."
      >
        &times;
      </button>
      ...
      >
        ...
        <div className="md:max-w-sm w-full bg-gray-800 bg-opacity-80 text-white p-4 rounded-lg flex-shrink-0">
          <h3 className="text-lg font-semibold border-b border-gray-600 pb-2 mb-3">
            생성 정보
          </h3>
                ...
                Object.entries(item.usedParameters).length > 0 && (
                  <div>
                    ...
                    <div className="mt-1 space-y-2 text-xs bg-black/20 p-3 rounded">
                      {Object.entries(item.usedParameters).map(
                        ([key, value]) => (
                          <div key={key}>
