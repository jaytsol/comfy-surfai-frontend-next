...
// --- 표준화된 ParameterMappingItem 인터페이스 ---
...
const ParameterMappingForm = ({
  ...
  const handleValueChange = (id: string, field: keyof ParameterMappingItem, value: any) => {
    setParameterMap(prevMap => prevMap.map(entry => 
      entry.id === id ? { ...entry, value: { ...entry.value, [field]: value } } : entry
    ));
  };
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">
            워크플로우 템플릿 '{template.name}'의 동적 파라미터를 설정합니다.
          </p>
        </div>
          ...
          <div key={entry.id} className="p-4 border rounded-lg space-y-4 bg-slate-50">
            <div className="flex justify-between items-center">
              <Input 
                ...
                className="font-mono font-bold text-lg w-1/3"
                placeholder="Parameter Key (e.g., positive_prompt)"
              />
            ...
            <div className="space-y-1">
              <Label>Label</Label>
              <Input value={entry.value.label} onChange={(e) => handleValueChange(entry.id, 'label', e.target.value)} placeholder="UI에 표시될 이름 (e.g., Positive Prompt)" />
            </div>
            <div className="space-y-1">
              <Label>Description</Label>
              <Textarea value={entry.value.description} onChange={(e) => handleValueChange(entry.id, 'description', e.target.value)} placeholder="파라미터에 대한 설명 (툴팁)" />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1">
                <Label>Type</Label>
                <select
                  ...
                  onChange={(e) => handleValueChange(entry.id, 'type', e.target.value as any)}
                  className="w-full p-2 border rounded bg-white"
                >
              ...
              <div className="space-y-1">
                ...
                <Input value={entry.value.default_value || ''} onChange={(e) => handleValueChange(entry.id, 'default_value', e.target.value)} placeholder="기본값 (선택 사항)" />
              </div>
            </div>
            {entry.value.type === 'select' && (
              <div className="space-y-1">
                ...
                <Input 
                  value={Array.isArray(entry.value.options) ? entry.value.options.join(', ') : entry.value.options} 
                  onChange={(e) => handleValueChange(entry.id, 'options', e.target.value)} 
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 items-center">
                <div className="flex items-center space-x-2">
                  <Checkbox 
                    ...
                    checked={entry.value.validation?.required} 
                    onCheckedChange={(checked) => handleValidationChange(entry.id, 'required', !!checked)}
                  />
                ...
                {entry.value.type === 'number' && (
                  <>
                    <div className="space-y-1">
                      <Label>Min</Label>
                      <Input type="number" value={entry.value.validation?.min || ''} onChange={(e) => handleValidationChange(entry.id, 'min', e.target.valueAsNumber)} />
                    </div>
                    <div className="space-y-1">
                      <Label>Max</Label>
                      <Input type="number" value={entry.value.validation?.max || ''} onChange={(e) => handleValidationChange(entry.id, 'max', e.target.valueAsNumber)} />
                    </div>
                    <div className="space-y-1">
                      <Label>Step</Label>
                      <Input type="number" value={entry.value.validation?.step || ''} onChange={(e) => handleValidationChange(entry.id, 'step', e.target.valueAsNumber)} />
                    </div>
                  </>
                )}
              </div>
