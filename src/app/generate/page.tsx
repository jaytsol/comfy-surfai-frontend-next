...
export default function GeneratePage() {
  ...
  const [generationResult, setGenerationResult] =
    useState<ImageGenerationData | null>(null);
  ...
  useEffect(() => {
    ...
    if (!user || user.role !== "admin") {
      if (ws.current) {
        console.log(
          "WebSocket: Closing due to user not admin or not logged in."
        );
        ws.current.close();
        ws.current = null;
    ...
    if (!ws.current) {
      ...
      const WEBSOCKET_SERVER_URL =
        process.env.NEXT_PUBLIC_WEBSOCKET_SERVER_URL || "wss://localhost:3000";
      const WEBSOCKET_PATH = "/generate";

      const socket = new WebSocket(
        `${WEBSOCKET_SERVER_URL}${WEBSOCKET_PATH}?clientId=${clientId}`
      );
      ws.current = socket;
      ...
      socket.onclose = (event) => {
        console.log(
          `WebSocket: Disconnected from ${WEBSOCKET_PATH}`,
          event.reason,
        ...
        if (ws.current) {
          // Check if ws.current was nulled by cleanup, meaning unmount
        ...
        try {
          ...
          switch (message.type) {
            case "progress":
              ...
              setExecutionStatus(
                `노드 ${progressData.node} 진행: ${progressData.value}/${progressData.max}`
              );
              break;
            case "status":
              const statusData = msgData as ComfyUIStatusData;
              if (
                statusData?.status?.exec_info?.queue_remaining !== undefined
              ) {
            ...
            case "executing":
              ...
              if (executingData.node === null && executingData.prompt_id) {
                setExecutionStatus(
                  `프롬프트 [${executingData.prompt_id.substring(
                    0,
                    8
                  )}...] 처리 완료.`
                );
            ...
            case "executed":
              ...
              if (executedData.output?.images) {
                const comfyUIBaseUrl =
                  ...
                  "https://comfy.surfai.org";
                const newPreviews = executedData.output.images
                  .filter((img) => img.type === "output")
                  .map((img) => ({
                    url: `${comfyUIBaseUrl}/view?filename=${encodeURIComponent(
                      img.filename
            ...
            case "execution_cached":
              ...
              setExecutionStatus(
                `프롬프트 [${cachedData.prompt_id.substring(
                  0,
                  8
                )}...]의 노드 (${cachedData.nodes.join(", ")})는 캐시됨.`
              );
              break;
            default:
    ...
    return () => {
      if (ws.current) {
        console.log(
          "WebSocket: Closing connection on component unmount or dependency change."
        );
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const response = await apiClient<ImageGenerationResponse>(
        "/api/generate",
        {
          method: "POST",
          body: payload,
      ...
      if (response.success && response.data) {
        if (response.data.prompt_id) {
          ...
          setExecutionStatus(
            "생성 작업은 시작되었으나 Prompt ID를 받지 못했습니다."
          );
        }
      } else {
    ...
    <div className="container mx-auto py-8 px-4">
      <div className="bg-white rounded-lg shadow-xl p-6 md:p-8">
        ...
        <p className="text-sm mb-4">
          WebSocket 연결 상태:{" "}
          <span className={isWsConnected ? "text-green-600" : "text-red-600"}>
        ...
        <LivePreviews previews={livePreviews} className="mt-6 mb-6" />
